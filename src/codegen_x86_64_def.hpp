...
namespace llgo
{
    namespace codegen
    {
        ...
        public:
            ...
            // --- TEST reg, reg (sets ZF=1 if reg==0, used for CondBr) ---
            void test_rr(X64Reg lhs, X64Reg rhs)
            {
                rexWRB(lhs, rhs);
                m_buf.emit8(0x85);
                modRM(3, lhs, rhs);
                    ...
                    switch (instr.kind)
                    {
                        ...
                        case NodeKind::CondBr:
                        {
                            if (instr.operandNames.size() >= 3)
                            {
                                ...
                                // TEST cond, cond: sets ZF=1 if cond==0 (false), ZF=0 if nonzero (true)
                                as.test_rr(cond, cond);
                                std::size_t truePos = as.jne_rel32_placeholder(); // jump if cond != 0
